...
const closeMenu = () => {
  ...
  gsap.to(navLinks, { x: 400, opacity: 0, duration: 0.4, ease: "power4.in" });
...
gsap.fromTo(
  ".new-product__small-images-img",
  { opacity: 0, y: 100, rotate: 45 },
  {
    opacity: 1,
    y: 0,
    duration: 0.6,
...
const newItemContainer = document.querySelector(
  ".client-bucket__products-container"
);
...
const addItem = (e) => {
  ...
  const itemPrice = parseFloat(
    prices.querySelector(".prices__current-price").textContent
  );
...
const closeBasketPageBtn = document
  .querySelector(".client-bucket__close ")
...
const removeItem = (id) => {
  ...
  const itemPrice = itemToRemove.children[2].querySelector(
    ".client-bucket__product-price"
  );
  const priceToRemove = parseFloat(itemPrice.textContent);
  const indexToRemove = totalSum.indexOf(priceToRemove);
  totalSum.splice(indexToRemove, 1);
  addedItemsNumber.textContent = totalSum.length - 1;
  const totalAmmountToPay = totalSum.reduce((a, b) => {
    return a + b;
  }, 0);
